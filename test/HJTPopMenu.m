...
@implementation UIView (HJTFrame)
...
- (void)dismiss
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(PopupMenuBeganDismiss)]) {
        [self.delegate PopupMenuBeganDismiss];
    }
...
+ (instancetype)showAtPoint:(CGPoint)point titles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth delegate:(id<HJTPopupMenuDelegate>)delegate
{
    HJTPopMenu *popupMenu = [[HJTPopMenu alloc] initWithTitles:titles icons:icons menuWidth:itemWidth delegate:delegate];
    [popupMenu showAtPoint:point];
    return popupMenu;
}

+ (instancetype)showRelyOnView:(UIView *)view titles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth delegate:(id<HJTPopupMenuDelegate>)delegate
{
    HJTPopMenu *popupMenu = [[HJTPopMenu alloc] initWithTitles:titles icons:icons menuWidth:itemWidth delegate:delegate];
...
- (void)setType:(HJTPopupMenuType)type
{
    _type = type;
    switch (type) {
        case HJTPopupMenuTypeDark:
        {
